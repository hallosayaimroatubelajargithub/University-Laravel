...
@section('title', 'Admin | Detail Matkul')
...
<div class="container">
    <div class="row">
        <div class="col-6">            
            <h3 class = "mt-3">Detail Mata Kuliah</h3>
            
            <div class="card">
                <div class="card-body">
                  <h5 class="card-title">{{$admatkuls->nama}}</h5>
                  <p class="card-text">Sks      : {{$admatkuls->sks}}</p>
                  <p class="card-text">Hari     : {{$admatkuls->hari}}</p>
                  <p class="card-text">Pukul    : {{$admatkuls->pukul}}</p>
                  <p class="card-text">Ruang    : {{$admatkuls->ruang}}</p>
                  
                  <a href="{{ $admatkuls->id }}/edit" class="btn btn-primary">Edit</a>
                  <form action="/admatkuls/{{ $admatkuls->id }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
                    <button type="submit" class="btn btn-danger">Delete</button>
                  </form>
                  <a href="/admatkuls" class="btn btn-warning">Kembali</a>
                </div>
              </div>
