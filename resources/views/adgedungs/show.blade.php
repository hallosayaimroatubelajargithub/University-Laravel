...
@section('title', 'Admin | Detail Data Gedung')
...
<div class="container">
    <div class="row">
        <div class="col-6">            
            <h3 class = "mt-3">Detail Gedung</h3>
            
            <div class="card">
                <div class="card-body">
                  <h5 class="card-title">{{$adgedungs->id_mhs}}</h5>
                  <p class="card-text">Nama Gedung      : {{$adgedungs->nama}}</p>
                  <a href="{{ $adgedungs->id }}/edit" class="btn btn-primary">Edit</a>
                  <form action="/adgedungs/{{ $adgedungs->id }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
                    <button type="submit" class="btn btn-danger">Delete</button>
                  </form>
                  <a href="/adgedungs" class="btn btn-warning">Kembali</a>
                </div>
              </div>
