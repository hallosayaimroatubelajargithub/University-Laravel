...
@section('title', 'Admin | Form Ubah Ruang')
    ...
    <div class="row">
        <div class="col-8">            
            <h3 class = "mt-3">Form Ubah Ruangan</h3>
                ...
                <form method="post" action="/adruangs/{{ $adruangs->id }}"><br>
                    @method('patch')
                    @csrf
                    <div class="mb-3">
                        <label for="penanggung_jawab" class="form-label">Nama Penanggung Jawab</label>
                        <input type="text" class="form-control @error('penanggung_jawab') is-invalid @enderror" id="penanggung_jawab" placeholder="Masukkan Nama Penanggung Jawab" name="penanggung_jawab" value="{{ $adruangs->penanggung_jawab }}">
                        @error('penanggung_jawab')<div class="invalid-feedback"> {{ $message }}</div>
                    </div>@enderror
