...
@section('title', 'Admin | Detail Dosen')
...
<div class="container">
    <div class="row">
        <div class="col-6">            
            <h3 class = "mt-3">Detail Dosen</h3>
            
            <div class="card">
                <div class="card-body">
                  <h5 class="card-title">{{$adosens->nama}}</h5>
                  <p class="card-text">Jenis Kelamin      : {{$adosens->jk}}</p>
                  <p class="card-text">Dosen Matkul     : {{$adosens->dosen_matkul}}</p>
                  
                  <a href="{{ $adosens->id }}/edit" class="btn btn-primary">Edit</a>
                  <form action="/adosens/{{ $adosens->id }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
                    <button type="submit" class="btn btn-danger">Delete</button>
                  </form>
                  <a href="/adosens" class="btn btn-warning">Kembali</a>
                </div>
              </div>
