...
@section('title', 'Admin | Detail Fakultas')
...
<div class="container">
    <div class="row">
        <div class="col-6">            
            <h3 class = "mt-3">Detail Fakultas</h3>
            
            <div class="card">
                <div class="card-body">
                  <h5 class="card-title">{{$adfaks->nama}}</h5>
                  <p class="card-text">Dekan      : {{$adfaks->dekan}}</p>
                  
                  <a href="{{ $adfaks->id }}/edit" class="btn btn-primary">Edit</a>
                  <form action="/adfaks/{{ $adfaks->id }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
                    <button type="submit" class="btn btn-danger">Delete</button>
                  </form>
                  <a href="/adfaks" class="btn btn-warning">Kembali</a>
                </div>
              </div>
