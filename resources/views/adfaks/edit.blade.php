...
@section('title', 'Admin | Form Ubah Fakultas')
...
<div class="container">
    <div class="row">
        <div class="col-8">            
            <h3 class = "mt-3">Form Ubah Fakultas</h3>
            
                <form method="post" action="/adfaks/{{ $adfaks->id }}"><br>
                    @method('patch')
                    @csrf
                    <div class="mb-3">
                        <label for="nama" class="form-label">Nama</label>
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" placeholder="Masukkan Nama" name="nama" value="{{ $adfaks->nama }}">
                        @error('nama')<div class="invalid-feedback"> {{ $message }}</div>
                    </div>@enderror

                    <div class="mb-3">
                        <label for="dekan" class="form-label">Dekan</label>
                        <input type="text" class="form-control @error('dekan') is-invalid @enderror" id="dekan" placeholder="Nama Dekan" name="dekan" value="{{ $adfaks->dekan }}">
                        @error('dekan')<div class="invalid-feedback"> {{ $message }}</div>
                    </div>@enderror
